...
export function reduce_target_position(state, { type, payload }) {
...
export function target_position({ client, world, events }) {
  for (const mob of world.mobs.all) {
    ...
    aiter(on(mob.events, 'state')).reduce((last_target, [{ target }]) => {
      if (last_target !== target) {
        console.log(last_target, target, client.uuid)
